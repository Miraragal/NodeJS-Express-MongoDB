...
campsiteRouter
  ...
  .delete(
    ...
    authenticate.verifyAdmin,
    (req, res, next) => {
      Campsite.findByIdAndRemove(req.params.campsiteId)
        .then((response) => {
          res.statusCode = 200;
          res.setHeader("Content-Type", "application/json");
          res.json(response);
    ...
    Campsite.findById(req.params.campsiteId)
      .then((campsite) => {
        if (campsite) {
          ...
          campsite
            .save() // save to updates to mongodb
            .then((campsite) => {
              res.statusCode = 200;
              res.setHeader("Content-Type", "application/json");
              res.json(campsite);
      ...
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
          if (
            ...
            campsite.save(); // save updates to mongodb
          } else {
      ...
      .then((campsite) => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
          if (
            ...
            campsite.save(); //save changes to mongodb
          } else {
            err = new Error("You are not authorized to delete this comment!");
            err.status = 403;
            return next(err);
